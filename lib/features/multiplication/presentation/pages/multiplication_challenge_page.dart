...
class _MultiplicationChallengePageState
  ...
  Widget build(BuildContext context) {
    final challengeState = ref.watch(multiplicationChallengeNotifierProvider);
    final challengeNotifier = ref.read(
      multiplicationChallengeNotifierProvider.notifier,
    );

    // チャレンジ完了時のリダイレクト
    ref.listen<MultiplicationChallengeState>(
      multiplicationChallengeNotifierProvider,
      (previous, next) {
        if (next.isChallengeComplete && mounted) {
          GoRouter.of(context).replace(
            '/result',
            extra: {
              'correctAnswers': next.correctAnswers,
              'totalProblems': next.totalProblems,
              // 星の数はResultPageで再計算するか、別途渡す
              'starsEarned': (next.correctAnswers == next.totalProblems)
                  ? 3
                  : (next.correctAnswers >= next.totalProblems * 0.7)
                  ? 2
                  : (next.correctAnswers >= next.totalProblems * 0.5)
                  ? 1
                  : 0,
            },
          );
          ...
          children: [
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                ...
                minimumSize: const Size(double.infinity, 60), // ボタンの幅を最大に
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(15),
                ),
              ),
  ...
  void _submitAnswer(MultiplicationChallengeNotifier notifier) {
    ...
    if (userAnswer != null) {
      ...
      _answerFocusNode.requestFocus(); // 次の問題のためにフォーカスを維持
    } else {
